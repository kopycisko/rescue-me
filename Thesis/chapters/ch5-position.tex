...
I am not sure: I would just say it depends on the spacing of the accesspoints, so you can always say that you are connected to the strongest (nearest one). In addition you can make estimations that you are "between" two accesspoints. So the worst case anticipation is accuracy = spacing of the accesspoints. Or in longer straight tunnels "Somewhere in between APx and APy".
So I disagree with what may be written in [31].
...
The figure \ref{fig:wifi_link_long} presents tests results performed on longer distance. On distances from $50$ up to $240$ meters the parameters of the link are similiar. Down link data remains at $40 MB$ and at $38 MB$ after $140$ meters with drops to $20 MB$. Up link data measurments are more unstable than in case of down link in therms of drops in delivered amount of data, but trend seems to be steady on range from $50$ till $220$ meters from access point. Then both data uplink and down link drops by $10 MB$. The data rate between access point and client device remain similiar from very beginning till distance around $370m$ where the signal is not strong enough to establish a connection.
