...
All of components listed above, except fignerprint sensor, can be used within the position finding system in order to give extra information about current physical state of the device. Compatibility definition guarantee some quality parameters of sensorics mounted in the mobile phone.
...
GPS/GNSS receiver are also not required component for a smartphone with Android 7.0 operating system. This receiver is also treated a one of the sensor. If the sensor is mounted then it is required to match set of time, reliability and accuracy constraints:
\begin{itemize}
...
Management of sensorics have to be implemented in a compliant manner in order to allow the operating system to interact with those sensorics. In case of Android 7.0, output of an accelerometer and magnetometer have to be normalized and presented to the system in the "Android sensor coordinate system" format. It means that the raw data are firstly filtered and then processed. Such prepared data are accesible for both operating system and the application developers.


\begin{itemize}
	\item What sensorics are present in mobile device.
...
\subsection{Abilities and limitations of mobile device in context of available positioning methods}
