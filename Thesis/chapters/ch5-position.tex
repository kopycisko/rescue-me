...
Modern underground machines and devices are equiped with computers. They are designed to ease machine configuration and its operations. They can also monitor work parameters and provide informations that can be usefull for service during periodical device checks or repairs. Positioning systems implementations may work togehter with these computers. Example of devices that make use positioning data are modern mobile machine gateways devices \cite{Thesis_CM}. Those computers can use positioning information in order to create reports of efficiency of their work expressed in for example load - unload cycles of LHD loaders (IREDES Performance Profile report). Computer recognise loading and dumping points by data provided by positioning system. It acumulates data about machine speed, distance traveled, time, amount of load that is carring and put into the report which creation is triggered by positioning information.


There is need to distinguish positioning systems with respect of the what is the target of acquired data. There can be defined three categories of positioning systems: positioning systems that are dedicated to acquire and transfer information about objects position to systems on surface, systems that are dedicated for on site usage to locate the device inside the underground installation and systems that combine both approaches.
